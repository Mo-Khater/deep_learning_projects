...
optmG = torch.optim.RMSprop(gen.parameters(),lr = lr)
optmD = torch.optim.RMSprop(dis.parameters(),lr = lr)
...
for epoch in range(num_epochs):
    ...
    for idx , (real_imgs,_) in enumerate(loader):
        for _ in range(critic_iterations):
            ...
            dis_loss =  - (torch.mean(dis_real_outputs.view(-1)) - torch.mean(dis_fake_ouputs.view(-1)))
            optmD.zero_grad()
            dis_loss.backward()
            optmD.step() 
